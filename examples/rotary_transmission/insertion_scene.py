...
def build(meshes: list[VolumeMesh], work_dir: str, settings: SimulationSettings) -> InsertionScene:
    builder = StarkSceneBuilder(settings, work_dir)
    sim = builder.simulation
    sim.set_gravity(np.zeros(3))
    candidates = sorted((builder.add_candidate(mesh, PLA_BASELINE) for mesh in meshes),
                        key=lambda body: float(body.rest_vertices_m[:, 0].mean()))
    centers = [(body.rest_vertices_m.min(0) + body.rest_vertices_m.max(0)) / 2 for body in candidates]
    ...
    shaft_i = solid_inertia(shaft_v, shaft_t, 0.020)
    input_shaft = presets.add("input_d_shaft", 0.020, shaft_i, shaft_v, shaft_t, contact_params)
    output_shaft = presets.add("output_d_connector", 0.020, shaft_i, shaft_v, shaft_t, contact_params)
    input_shaft.rigidbody.set_translation(np.array([centers[0][0], 0.0, -0.018]))
    output_shaft.rigidbody.set_translation(np.array([centers[1][0], 0.0, 0.018]))
    input_press = sim.rigidbodies().add_constraint_prismatic_press(
        ground.rigidbody, input_shaft.rigidbody, np.array([centers[0][0], 0.0, -0.018]),
        np.array([0.0, 0.0, 1.0]), 0.002, 35.0)
    output_press = sim.rigidbodies().add_constraint_prismatic_press(
        ground.rigidbody, output_shaft.rigidbody, np.array([centers[1][0], 0.0, 0.018]),
        np.array([0.0, 0.0, -1.0]), 0.002, 35.0)
